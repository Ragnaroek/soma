...
fn exec_ld_to_h_from_immediate(sm83: &mut SM83, mc: &mut MemoryController) -> Result<(), ExecErr> {
    let val = mc.read(sm83.pc() + 1)?;
    sm83.reg.h = val;
    sm83.inc_pc(2);
    Ok(())
...
fn exec_ld_to_deref_hl_from_a(sm83: &mut SM83, mc: &mut MemoryController) -> Result<(), ExecErr> {
    let addr = sm83.reg.hl();
    mc.write(addr, sm83.reg.a)?;
    sm83.inc_pc(1);
    Ok(())
...
// INC
fn exec_inc_a(sm83: &mut SM83, _: &mut MemoryController) -> Result<(), ExecErr> {
    let (a_inc, _) = sm83.reg.a.overflowing_add(1);
    let half_carry = half_carry_inc(sm83.reg.a);
    sm83.reg.a = a_inc;
    sm83.reg.set_flag(Z, (a_inc == 0) as u8);
    sm83.reg.set_flag(N, 0);
...
fn exec_inc_l(sm83: &mut SM83, _: &mut MemoryController) -> Result<(), ExecErr> {
    let (l_inc, _) = sm83.reg.l.overflowing_add(1);
    let half_carry = half_carry_inc(sm83.reg.l);
    sm83.reg.l = l_inc;
    sm83.reg.set_flag(Z, (l_inc == 0) as u8);
    sm83.reg.set_flag(N, 0);
    sm83.reg.set_flag(H, half_carry);
    sm83.inc_pc(1);
    Ok(())
...
fn exec_inc_deref_hl(sm83: &mut SM83, mc: &mut MemoryController) -> Result<(), ExecErr> {
    let addr = sm83.reg.hl();
    let v = mc.read(addr)?;
    let half_carry = half_carry_inc(v);
    let (v_inc, _) = v.overflowing_add(1);
    mc.write(addr, v_inc)?;
    sm83.reg.set_flag(Z, (v_inc == 0) as u8);
    sm83.reg.set_flag(N, 0);
    sm83.reg.set_flag(H, half_carry);
    sm83.inc_pc(1);
    Ok(())
...
fn exec_dec_a(sm83: &mut SM83, _mc: &mut MemoryController) -> Result<(), ExecErr> {
    let (a_dec, _) = sm83.reg.a.overflowing_sub(1);
    sm83.reg.a = a_dec;
    sm83.reg.set_flag(Z, (a_dec == 0) as u8);
    sm83.reg.set_flag(N, 1);
    sm83.reg.set_flag(H, half_carry_dec(a_dec));
    sm83.inc_pc(1);
    Ok(())
}
...
fn exec_dec_h(sm83: &mut SM83, _mc: &mut MemoryController) -> Result<(), ExecErr> {
    let (h_dec, _) = sm83.reg.h.overflowing_sub(1);
    sm83.reg.h = h_dec;
    sm83.reg.set_flag(Z, (h_dec == 0) as u8);
    sm83.reg.set_flag(N, 1);
    sm83.reg.set_flag(H, half_carry_dec(h_dec));
    sm83.inc_pc(1);
    Ok(())
}
...
fn exec_or_a_d(sm83: &mut SM83, _mc: &mut MemoryController) -> Result<(), ExecErr> {
    sm83.reg.a = sm83.reg.a | sm83.reg.d;
    sm83.reg.set_flag(Z, (sm83.reg.a == 0) as u8);
    sm83.reg.set_flag(N, 0);
    sm83.reg.set_flag(H, 0);
    sm83.reg.set_flag(C, 0);
...
fn exec_a_and_a(sm83: &mut SM83, _mc: &mut MemoryController) -> Result<(), ExecErr> {
    sm83.reg.set_flag(Z, (sm83.reg.a == 0) as u8);
    sm83.reg.set_flag(N, 0);
    sm83.reg.set_flag(H, 1);
    sm83.reg.set_flag(C, 0);
...
fn exec_a_and_c(sm83: &mut SM83, _mc: &mut MemoryController) -> Result<(), ExecErr> {
    sm83.reg.a = sm83.reg.a & sm83.reg.c;
    sm83.reg.set_flag(Z, (sm83.reg.a == 0) as u8);
    sm83.reg.set_flag(N, 0);
    sm83.reg.set_flag(H, 1);
...
fn exec_ret_nz(sm83: &mut SM83, mc: &mut MemoryController) -> Result<(), ExecErr> {
...
fn exec_pop_de(sm83: &mut SM83, mc: &mut MemoryController) -> Result<(), ExecErr> {
    let lsb = mc.read(sm83.reg.sp)?;
    sm83.inc_sp(1);
    let msb = mc.read(sm83.reg.sp)?;
    sm83.inc_sp(1);
...
fn exec_push_hl(sm83: &mut SM83, mc: &mut MemoryController) -> Result<(), ExecErr> {
    let addr = sm83.reg.hl().to_le_bytes();
    sm83.dec_sp(1);
    mc.write(sm83.reg.sp, addr[1])?;
    sm83.dec_sp(1);
    mc.write(sm83.reg.sp, addr[0])?;
...
pub static EXEC_TABLE: [Sm83Exec; psy::arch::sm83::SM83_NUM_INSTRUCTIONS] = [
    ...
    /*0x12*/ exec_ld_to_deref_de_from_a,
    /*0x13*/ exec_inc_de,
    /*0x14*/ exec_invalid,
    /*0x15*/ exec_invalid,
    ...
    /*0x75*/ exec_invalid,
    /*0x76*/ exec_invalid,
    /*0x77*/ exec_ld_to_deref_hl_from_a,
